...
## 4.2 Age Group & Customer Segment Distribution

This visualisation illustrates the distribution of customer segments across different age groups. It helps identify which age groups dominate specific customer segments.
...
ggplot(customer_data,
  ...
  scale_fill_manual(values = c("inactive" = "#95A5A6",
                               ...
                               "regular" = "#2ECC71")) +
  theme_minimal() +
  labs(title = "Customer Transaction Frequency by Age Group and Segment",
       x = "Age Group",
       y = "Transaction Frequency (log scale)")
```

Customer engagement level appears to have a stronger influence on transaction behaviour than demographic factors such as age. The log scale helps adjust the highly skewed distribution of transaction activity.

## 4.3 Customer Behaviour Analysis
...
## 4.4 Financial Product Adoption by Customer Segment
...
## 4.5 Behavioural Patterns Across Cities
...
The analysis provides several key insights into the structure of the fintech platform's customer base.

First, demographic analysis reveals the distribution of customers across different age groups and customer segments.

Second, behavioural analysis shows that transaction frequency and transaction value vary significantly across customer segments, highlighting differences in engagement levels.

Third, product adoption analysis demonstrates that certain financial products are more commonly used by specific customer segments, suggesting opportunities for targeted marketing and cross-selling.

Finally, the geographic behaviour heatmap reveals regional differences in fintech engagement across Colombian cities.
