...
```{r}
# Note: odbcConnectAccess2007() is Windows-only.
# On macOS, CoffeeChain.rds is loaded directly instead.
coffeechain <- readRDS("CoffeeChain.rds")
```
...
The code chunk is used to plot the bullet charts using ggplot2 functions.
...
It is important to note that one of the requirement of gtExtras functions is that almost exclusively they require to pass data.frame with list columns. In view of this, code chunk below will be used to convert the report data.frame into list columns.
...
Next, adding the statistics on the table.
...
Similarly, combining the bullet chart and sparklines.
...
Similar to gtExtras, to plot an interactive sparklines by using reactablefmtr package need to prepare the list field.
